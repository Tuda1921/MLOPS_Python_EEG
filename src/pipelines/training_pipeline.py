...
import logging
def training_pipeline(file_name: str, time_steps: int, units, dropout_rate, epochs, batch_size, patience, learning_rate, fc_units):
    try:
        ...
        input_shape = (X_train.shape[1], time_steps//2+1)  # (6768, 10)

        # Khởi tạo model LSTM với các thông số đã cho và input_dim tự động
        model = LSTMModel(input_shape=input_shape, units=units, dropout_rate=dropout_rate, num_classes=3, learning_rate=learning_rate, fc_units=fc_units)
